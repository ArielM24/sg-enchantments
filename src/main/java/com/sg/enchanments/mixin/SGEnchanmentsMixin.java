...
public abstract class SGEnchanmentsMixin {
    ...
    private static boolean hookCanBeCombined(boolean original, RegistryEntry<Enchantment> first, RegistryEntry<Enchantment> second){
		...
		if((SGEnchanments.isSameEnchanment(first.value(), Enchantments.INFINITY) && SGEnchanments.isSameEnchanment(second.value(), Enchantments.MENDING))||(SGEnchanments.isSameEnchanment(first.value(), Enchantments.MENDING) && SGEnchanments.isSameEnchanment(second.value(), Enchantments.INFINITY))){
			return true;
		}
		return original;
	}
